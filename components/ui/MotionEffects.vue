...
function initMotion() {
  ...
  const tiltTargets = document.querySelectorAll<HTMLElement>('article, main a.group')
  ...
  const hero = document.querySelector<HTMLElement>('main > div > section:first-child img, main > section:first-child img')
  if (hero) {
    const parallax = () => hero.style.setProperty('--hero-shift', `${Math.min(window.scrollY * 0.13, 75)}px`)
...
function ripple(event: MouseEvent) {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return
  const target = (event.target as HTMLElement).closest<HTMLElement>('button, .btn-primary, .btn-light')
  if (!target) return
...
watch(() => route.fullPath, () => nextTick(initMotion))
onBeforeUnmount(() => { cleanup(); document.removeEventListener('click', ripple) })
</script>
